  ...
  "mcpServers": {
    "toshers": {
      ...
      "url": "https://toshers.vercel.app/api/mcp"
        ...
        <p style={{marginTop: '1.5rem'}}>Once mounted, the agent gains nineteen tools plus five prompts. Every tool description carries a <code>cost:</code> tag so agents can budget calls — only <code>semantic_search</code> is expensive.</p>
        <div className="endpoint-grid">
          <div className="endpoint"><span className="method">tool</span><span className="path">get_index(depth?)</span><span className="desc">Master catalogue — call first to orient. <code>depth=&quot;shallow&quot;</code> for ~2kb nav-only variant; <code>&quot;full&quot;</code> (default) for full catalogue with INDEX.md</span></div>
          ...
          <div className="endpoint"><span className="method">tool</span><span className="path">get_chapter(id, profile?)</span><span className="desc">Full chapter file by id. <code>profile=&quot;minimal&quot;|&quot;facts&quot;|&quot;full&quot;</code></span></div>
          <div className="endpoint"><span className="method">tool</span><span className="path">get_chapter_annotated(id)</span><span className="desc">Chapter body + span offsets for glossary terms and pre-decimal prices</span></div>
          <div className="endpoint"><span className="method">tool</span><span className="path">list_characters</span><span className="desc">Lightweight testimony metadata only (includes <code>mayhew</code>)</span></div>
          <div className="endpoint"><span className="method">tool</span><span className="path">get_character(id, profile?)</span><span className="desc">Full first-person testimony by id. <code>profile=&quot;minimal&quot;</code> omits body; <code>&quot;facts&quot;</code> drops game_hooks</span></div>
          <div className="endpoint"><span className="method">tool</span><span className="path">get_character_annotated(id)</span><span className="desc">Testimony body + span offsets — drop into a reader UI with glossary and currency popovers</span></div>
          ...
          <div className="endpoint"><span className="method">tool</span><span className="path">voice_profile(id)</span><span className="desc">TTS profile — dialect level, accent hint, IPA <code>pronunciation_overrides</code>, <code>suggested_voice_model</code>, <code>ssml_hints</code></span></div>
          <div className="endpoint"><span className="method">tool</span><span className="path">list_quotes(speaker_id?, chapter_ref?, theme?, dialect_level?)</span><span className="desc">Verbatim pulled-quotes + TTS-normalised rendition. Response includes <code>coverage</code> — curation is NOT exhaustive</span></div>
          ...
          <div className="endpoint"><span className="method">tool</span><span className="path">normalize_price(pounds?, shillings?, pence?, literal?)</span><span className="desc">Pre-decimal £/s/d → decimal 1851 pounds → four modern-GBP equivalents (CPI, labour value, income value, GDP share). They can differ by an order of magnitude. Also accepts a string <code>literal</code> like <code>&quot;£3 5s 6d&quot;</code> or Mayhew&apos;s <code>&quot;3l. 5s.&quot;</code> notation.</span></div>
      ...
      <section>
        ...
        <div className="endpoint-grid">
          <div className="endpoint"><span className="method">GET</span><span className="path"><a href="/api/index">/api/index</a></span><span className="desc">Master catalogue (chapters + characters + line counts + indexes). Add <code>?depth=shallow</code> for a ~2kb nav-only variant. Returns <code>version</code> and git <code>commit</code> for cache invalidation</span></div>
          ...
          <div className="endpoint"><span className="method">GET</span><span className="path">/api/chapters/{`{id}`}</span><span className="desc">One chapter. <code>?format=raw</code> for raw markdown; <code>?profile=minimal|facts|full</code> for payload control</span></div>
          <div className="endpoint"><span className="method">GET</span><span className="path">/api/chapters/{`{id}`}/annotated</span><span className="desc">Chapter body + inline spans for glossary terms and pre-decimal prices</span></div>
          <div className="endpoint"><span className="method">GET</span><span className="path"><a href="/api/characters">/api/characters</a></span><span className="desc">List all character testimonies with metadata (includes <code>mayhew</code> — the narrator dossier)</span></div>
          <div className="endpoint"><span className="method">GET</span><span className="path">/api/characters/{`{id}`}</span><span className="desc">One testimony. <code>?format=raw</code>; <code>?profile=minimal|facts|full</code></span></div>
          <div className="endpoint"><span className="method">GET</span><span className="path">/api/characters/{`{id}`}/annotated</span><span className="desc">Testimony body + inline spans — feeds a reader UI with glossary and currency popovers</span></div>
          <div className="endpoint"><span className="method">GET</span><span className="path"><a href="/api/characters/06-cuckolds-point-tosher/voice">/api/characters/{`{id}`}/voice</a></span><span className="desc">TTS profile — dialect level, accent hint, speech notes, IPA <code>pronunciation_overrides</code>, <code>suggested_voice_model</code>, <code>ssml_hints</code></span></div>
          ...
          <div className="endpoint"><span className="method">GET</span><span className="path"><a href="/api/prices/normalize?pounds=3&shillings=5">/api/prices/normalize?pounds={`{n}`}&amp;shillings={`{n}`}&amp;pence={`{n}`}</a></span><span className="desc">Pre-decimal £/s/d → decimal 1851 pounds → four modern-GBP equivalents (CPI, labour value, income value, GDP share) — an order of magnitude apart. Also accepts <code>?literal=£3+5s+6d</code> or Mayhew&apos;s <code>3l. 5s.</code> notation</span></div>
